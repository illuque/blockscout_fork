defmodule BlockScoutWeb.API.V2.AddressControllerTest do
  ...
  describe "checks TokenBalanceOnDemand" do
    ...
    test "Indexer.Fetcher.TokenBalanceOnDemand broadcasts only updated balances", %{conn: conn} do
      ...
      block_number_hex = ("0x" <> Integer.to_string(insert(:block).number, 16)) |> String.downcase()
      ...
      expect(EthereumJSONRPC.Mox, :json_rpc, fn [
        ...
        result_1 =
          balances_erc_1155[{contract_address_1, to_string(token_id_1)}]
          |> List.wrap()
        ...
        result_2 =
          balances_erc_1155[{contract_address_2, to_string(token_id_2)}]
          |> List.wrap()
          |> TypeEncoder.encode_raw([{:uint, 256}], :standard)
          |> Base.encode16(case: :lower)
      ...
      expect(EthereumJSONRPC.Mox, :json_rpc, fn [
        ...
        assert request_1 |> Base.encode16!(case: :lower) |> TypeDecoder.decode_raw(types_list) == [
                 to_string(address.hash)
               ]

        assert request_2 |> Base.encode16!(case: :lower) |> TypeDecoder.decode_raw(types_list) == [
                 to_string(address.hash)
               ]

        assert request_3 |> Base.encode16!(case: :lower) |> TypeDecoder.decode_raw(types_list) == [
                 to_string(address.hash)
               ]

        assert request_4 |> Base.encode16!(case: :lower) |> TypeDecoder.decode_raw(types_list) == [
                 to_string(address.hash)
               ]

        result_1 =
          other_balances[contract_address_1]
          |> List.wrap()
          |> TypeEncoder.encode_raw([{:uint, 256}], :standard)
          |> Base.encode16(case: :lower)

        result_2 =
          other_balances[contract_address_2]
          |> List.wrap()
          |> TypeEncoder.encode_raw([{:uint, 256}], :standard)
          |> Base.encode16(case: :lower)

        result_3 =
          other_balances[contract_address_3]
          |> List.wrap()
          |> TypeEncoder.encode_raw([{:uint, 256}], :standard)
          |> Base.encode16(case: :lower)

        result_4 =
          other_balances[contract_address_4]
          |> List.wrap()
          |> TypeEncoder.encode_raw([{:uint, 256}], :standard)
          |> Base.encode16(case: :lower)
      ...
      assert Decimal.to_integer(ctb_erc_20["value"]) == other_balances[ctb_erc_20["token"]["address"]]
      assert Decimal.to_integer(ctb_erc_721["value"]) == other_balances[ctb_erc_721["token"]["address"]]

      assert Decimal.to_integer(ctb_erc_1155["value"]) ==
               balances_erc_1155[{ctb_erc_1155["token"]["address"], to_string(ctb_erc_1155["token_id"])}]
    end
  end
